...
(function () {
  ...
  function _showDiffView(original, result, opts) {
    // Remove existing
    document.querySelectorAll('.ldoc-ai-diff-panel').forEach(function (p) { p.remove(); });

    var panel = document.createElement('div');
    panel.className = 'ldoc-ai-diff-panel';
    ...
    header.className = 'ldoc-ai-diff-header';
    header.innerHTML =
      '<span>AI Suggestion</span>' +
      '<div class="ldoc-ai-diff-actions">' +
        '<button class="ldoc-ai-diff-btn ldoc-ai-diff-accept" title="Accept"><span class="material-icons" style="font-size:16px">check</span> Accept</button>' +
        '<button class="ldoc-ai-diff-btn ldoc-ai-diff-reject" title="Reject"><span class="material-icons" style="font-size:16px">close</span> Reject</button>' +
      '</div>';
    panel.appendChild(header);

    var body = document.createElement('div');
    body.className = 'ldoc-ai-diff-body';

    var diff = _computeWordDiff(original, result);

    // Original column
    var colOrig = document.createElement('div');
    colOrig.className = 'ldoc-ai-diff-col';
    colOrig.innerHTML = '<div class="ldoc-ai-diff-col-label">Original</div>';
    var origContent = document.createElement('div');
    origContent.className = 'ldoc-ai-diff-content';
    for (var k = 0; k < diff.length; k++) {
      var d = diff[k];
      if (d.type === 'same') origContent.appendChild(document.createTextNode(d.text));
      else if (d.type === 'removed') {
        var span = document.createElement('span');
        span.className = 'ldoc-ai-diff-removed';
    ...
    colResult.className = 'ldoc-ai-diff-col';
    colResult.innerHTML = '<div class="ldoc-ai-diff-col-label">Suggested</div>';
    var resultContent = document.createElement('div');
    resultContent.className = 'ldoc-ai-diff-content';
    for (var k = 0; k < diff.length; k++) {
      var d = diff[k];
      if (d.type === 'same') resultContent.appendChild(document.createTextNode(d.text));
      else if (d.type === 'added') {
        var span = document.createElement('span');
        span.className = 'ldoc-ai-diff-added';
        span.textContent = d.text;
    ...
    header.querySelector('.ldoc-ai-diff-accept').addEventListener('click', function () {
      if (opts && opts.onAccept) opts.onAccept(result);
      panel.remove();
    });

    // Reject — just close
    header.querySelector('.ldoc-ai-diff-reject').addEventListener('click', function () {
      panel.remove();
    });
  ...
  function _fireAIEdit(action, customPrompt, selectedText, range, sel, editableEl, opts, clickX, clickY) {
    ...
    loadingEl.className = 'ldoc-ai-ctx-loading';
    if (cv2IsDark()) loadingEl.classList.add('cv2-dark');
    loadingEl.innerHTML =
      '<span class="ldoc-ai-task-spinner"></span> ' + _t('ai.thinking') +
      '<button class="ldoc-ai-cancel-btn" title="' + _t('ai.cancel') + '"><span class="material-icons" style="font-size:14px">close</span></button>';
    document.body.appendChild(loadingEl);
    cv2PositionPopup(loadingEl, clickX, clickY);

    var _cancelled = false;

    loadingEl.querySelector('.ldoc-ai-cancel-btn').addEventListener('click', function () {
      _cancelled = true;
  ...
  async function _renderMarkdownForDoc(markdown, docType) {
    ...
    var mermaidContainers = temp.querySelectorAll('.ldoc-mermaid-container');
    for (var i = 0; i < mermaidContainers.length; i++) {
      ...
      if (svg) {
        ...
        var block = mc.closest('.ldoc-plugin-block') || mc.parentNode;
        block.replaceWith(img);
      }
    }

    // Plotly → render to PNG (white bg for emails)
    var plotlyEls = temp.querySelectorAll('.ldoc-plotly-preview');
    for (var j = 0; j < plotlyEls.length; j++) {
      ...
      if (window.Plotly && pe.data) {
        try {
          ...
          var plotBlock = pe.closest('.ldoc-plugin-block') || pe.parentNode;
    ...
    var remaining = temp.querySelectorAll('.ldoc-plugin-block');
  ...
  function _buildAITaskButton(toolbar, opts) {
    ...
    btn.className = 'ldoc-rich-toolbar-btn ldoc-ai-task-btn';
    ...
    btn.addEventListener('click', function () {
      ...
      inputWrap.className = 'ldoc-ai-task-input';
      inputWrap.innerHTML =
        '<input type="text" placeholder="Describe what to generate\u2026" class="ldoc-ai-task-field" />' +
        '<button class="ldoc-ai-task-go" title="Generate"><span class="material-icons" style="font-size:16px">send</span></button>';

      // Insert after toolbar
      toolbar.parentNode.insertBefore(inputWrap, toolbar.nextSibling);

      var field = inputWrap.querySelector('.ldoc-ai-task-field');
      var goBtn = inputWrap.querySelector('.ldoc-ai-task-go');
      field.focus();
      ...
      function doGenerate() {
        ...
        field.disabled = true;
        goBtn.innerHTML =
          '<span class="ldoc-ai-task-spinner"></span>';

        // Add cancel button next to spinner
        var cancelBtn = document.createElement('button');
        cancelBtn.className = 'ldoc-ai-cancel-btn';
    ...
    var sep = toolbar.querySelector('.ldoc-rich-toolbar-sep:last-of-type');
  ...
  function _setupGhostText(editableEl, opts) {
    ...
    function onSuggestion(ev) {
      ...
      _ghostSpan.className = 'ldoc-ghost-text';
