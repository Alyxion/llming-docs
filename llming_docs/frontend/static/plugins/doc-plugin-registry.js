...
class DocPluginRegistry {
   ...
   * @param {boolean} [opts.fencedBlockAllowed=true] — whether the markdown
   *   renderer should render this lang when seen as a fenced code block.
   *   Persistent document types (plotly, text_doc, table, presentation,
   *   html_sandbox, latex, email_draft, word, powerpoint, pptx) set this to
   *   false under the tool-only policy: documents must be created via the
   *   create_document MCP tool, never via fenced code blocks in the response.
   *   Tool-driven rendering (via _injectToolDocBlock calling render directly)
   *   still works regardless of this flag.
   * @param {Function} [opts.loader] — async function to load external libs before first render
   */
  register(lang, { render, inline = true, sidebar = true, fencedBlockAllowed = true, loader }) {
    this._plugins.set(lang, { render, inline, sidebar, fencedBlockAllowed, loaded: !loader, loader });
  }
  ...
  /** Whether the markdown code-block renderer should produce a plugin
   *  placeholder for this lang. Returns false for doc types (tool-only). */
  fencedBlockAllowed(lang) {
    const p = this._plugins.get(lang);
    return !!(p && p.fencedBlockAllowed !== false);
  }
